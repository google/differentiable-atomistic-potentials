...
If you notice that your programs seem to slow down, it could be you are inadvertently accumulating nodes in the graph. For example, here is a bad way to run a program.
...
for i in range(5):
    ...
    with tf.Session() as sess:       
        val = a.eval()
        print('# nodes: ', len([n.name for n in tf.get_default_graph().as_graph_def().node]), 'a: ', val)
#+END_SRC
...
Note the number of nodes is increasing, which is not desirable. Here is the right way, using a placeholder.
...
Now, note that the number of nodes is constant.
