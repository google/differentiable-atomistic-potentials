...
def energy(positions, numbers, cell, strain=np.zeros((3, 3))):
    ...
    all_neighbors, all_offsets = get_neighbors_oneway(positions, cell,
                                                      rc_list, skin=0.0)
    ...
    for a1 in range(natoms):
        ...
        for a2, offset in zip(neighbors, offsets):
            ...
            if r < rc_list:
                ...
                theta = 1.0 / (1.0 + x)
                y1 = (0.5 * p1['V0'] *
                      np.exp(-p2['kappa'] * (r / beta - p2['s0'])) *
                      _ksi[Z2] / p1['gamma2'] * theta)
                y2 = (0.5 * p2['V0'] *
                      np.exp(-p1['kappa'] * (r / beta - p1['s0'])) /
                      _ksi[Z2] / p2['gamma2'] * theta)
                energy = energy - (y1 + y2)

                sa = (np.exp(-p2['eta2'] * (r - beta * p2['s0'])) *
                      _ksi[Z2] * theta / p1['gamma1'])
                sigma1[a1] = sigma1[a1] + sa

                sa = (np.exp(-p1['eta2'] * (r - beta * p1['s0'])) /
                      _ksi[Z2] * theta / p2['gamma1'])
